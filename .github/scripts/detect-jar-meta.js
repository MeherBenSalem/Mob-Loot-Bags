 ...
 *   mob_loot_bags-{loader}-{mcVersion}-{modVersion}.jar
 * Legacy convention:
...
function detectRequired(jar) {
  ...
  throw new Error(
    `Could not detect Minecraft version from jar name: ${path.basename(jar)}. ` +
      `Expected mob_loot_bags-{loader}-{mcVersion}-{modVersion}.jar`,
  );
}
...
if (require.main === module) {
  ...
  if (!jar) {
    console.error('Usage: detect-jar-meta.js <jar-path-or-name>');
    process.exit(2);
  }
